...
function InnerWrapper({ children }: { children: React.ReactNode }) {
  ...
  if (isLandingOrAuth || pathname.startsWith('/trade/')) {
    return (
      <div
        className="flex"
        style={{
          ...
          alignItems: isLandingOrAuth && pathname !== '/' && pathname !== '/frontpage' ? 'center' : 'stretch',
          justifyContent: isLandingOrAuth && pathname !== '/' && pathname !== '/frontpage' ? 'center' : 'stretch',
          padding: 0,
        }}
      >
        <div style={{ width: '100%', maxWidth: isLandingOrAuth ? 'none' : '1400px', margin: '0 auto' }}>
          {children}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex" style={{ minHeight: '100vh' }}>
      ...
      >
        <Link href="/frontpage" className="logo flex items-center gap-2" style={{ textDecoration: 'none', color: 'inherit' }}>
          <div
            style={{
              width: '32px',
              height: '32px',
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: 700 }}>
            TradeLens<span style={{ color: 'var(--text-secondary)', fontWeight: 400 }}>.trade</span>
          </h2>
        </Link>
        ...
        <div style={{ position: 'relative' }}>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              >
                {(user?.username || activeAccount).charAt(0).toUpperCase()}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>{user?.username || activeAccount}</span>
                <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>
                  {user?.role === 'admin' ? 'Administrator' : 'Live Account'}
                </span>
              </div>
            </div>
            <svg
          ...
          {isDropdownOpen && (
            ...
            >
              {['Exness MT5', 'Exness MT4', 'Personal IBKR', 'Apex Prop 50k'].map((acc) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '8px',
                    cursor: 'pointer',
                    background: activeAccount === acc ? 'rgba(255,255,255,0.1)' : 'transparent',
                    fontSize: '0.85rem',
                  }}
              ...
              <div
                style={{ padding: '10px 12px', borderRadius: '8px', cursor: 'pointer', fontSize: '0.85rem', color: 'var(--text-secondary)' }}
                className="hover:bg-white/5"
              >
                + Add Account
              </div>
        ...
        <nav className="flex flex-col gap-2">
          <Link href="/dashboard" className="nav-item" style={navItemStyle(pathname === '/dashboard')}>
          ...
          <div className="glass-panel" style={{ padding: '16px' }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <span style={{ fontSize: '0.9rem', fontWeight: 500 }}>
                {activeAccount.includes('Prop') ? 'Rithmic (Live)' : 'Interactive Brokers'}
              </span>
            </div>
          </div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              ...
              <svg
                ...
                stroke="currentColor"
                strokeWidth="2"
                style={{ transform: isThemeMenuOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}
              >
                <polyline points="6 9 12 15 18 9"></polyline>
              </svg>
            </div>
                ...
                {['dark', 'light', 'midnight', 'cyberpunk'].map((theme) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: '8px',
                      cursor: 'pointer',
                      background: activeTheme === theme ? 'rgba(100,100,100,0.2)' : 'transparent',
                      fontSize: '0.85rem',
